...
@st.cache_resource
def load_models():
    with open('/Users/Playdata/Desktop/SKN06-2nd-7Team/Model/best_lr.pkl', 'rb') as f:  # 머신러닝 모델 불러오기 
        ml_model = pickle.load(f)
        print(type(ml_model))

    input_size = 38
    dl_model = Net(input_size)
    dl_model.load_state_dict(torch.load('/Users/Playdata/Desktop/SKN06-2nd-7Team/Model/best_model.pth', map_location=torch.device('cpu')))  # 딥러닝 모델 불러오기 
    dl_model.eval()
...
def load_preprocessor():
    ...
    return joblib.load('/Users/Playdata/Desktop/SKN06-2nd-7Team/Model/preprocessor_pipeline.pkl')

def load_f_data():
    return pd.read_csv('/Users/Playdata/Desktop/SKN06-2nd-7Team/Data/f_data.csv')
...
if "select_model" not in st.session_state:
    ...
    st.write(f"예측 결과: {'이직 가능성 있음' if prediction_value > 0.5 else '이직 가능성 낮음'}")
